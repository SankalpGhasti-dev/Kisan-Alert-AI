...
async def build_dashboard_data(state: str, district: str) -> dict:
    """
    ...
    # ── 1. Soil lookup ──
    soil_row = soil_df[
        (soil_df["state"] == state) & (soil_df["district"] == district)
    ]
    if soil_row.empty:
        raise HTTPException(
            status_code=404,
            detail=f"Soil data not found for {district}, {state}",
        )
